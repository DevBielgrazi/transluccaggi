...
$ano_fre = trim($_POST['ano_fre']);
$mes_fre = trim($_POST['mes_fre']);
$dis_fre = trim($_POST['dis_fre']);

$dat_fre = $ano_fre."-".$mes_fre."-01";
$dat_fre2 = $ano_fre."-".$mes_fre."-31";

require('../connect.php');
#ADQUIRINDO INFORMAÇÕES DO BANCO
if($dis_fre >1){
    $sql = mysqli_query($conn,"SELECT SUM(`valor`) as 'val' FROM $tab_nfs WHERE `cod_distribuidora` = '$dis_fre' AND `entrada` >= '$dat_fre' AND `entrada` <= '$dat_fre2'");
}
else{
    $sql = mysqli_query($conn,"SELECT SUM(`valor`) as 'val' FROM $tab_nfs WHERE `cod_distribuidora` = '$dis_fre' AND `emissao` >= '$dat_fre' AND `emissao` <= '$dat_fre2'");
}
$sql = mysqli_fetch_array($sql);
#VARIÁVEL DO BANCO
$val_mes = number_format(($sql['val']), 2, '.', '');

$sql2 = mysqli_query($conn,"SELECT * FROM $tab_dis WHERE `codigo` = '$dis_fre'");
...
?>
		<urd>
			<table border=1>
                ...
                <tr>
					<td><h4><nobr><?php echo $mes_fre."/".$ano_fre;   ?><nobr></h4></td>
